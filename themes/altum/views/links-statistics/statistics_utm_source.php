    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center">
                <h3 class="h5 text-truncate m-0"><?= l('link_statistics.utms') ?></h3>

                <div class="ml-2">
                    <span data-toggle="tooltip" title="<?= l('link_statistics.utms_help') ?>">
                    ...
                    <div class="d-flex justify-content-between mb-1">
                        <div class="text-truncate">
                            <?php if(!empty($row->utm_source)): ?>
                                <a href="<?= url('links-statistics?' . $data->filters->get_get() . '&type=utm_medium&utm_source=' . $row->utm_source . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>"><?= $row->utm_source ?></a>
                            <?php else: ?>
                                <?= l('global.none') ?>
                            <?php endif ?>
                        </div>
